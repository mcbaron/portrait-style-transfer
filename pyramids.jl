...
type ImagePyramid
    ...
    function ImagePyramid(im::AbstractArray, t::ComplexSteerablePyramid; scale=0.5, min_size=15, num_orientations=8, max_levels=23, twidth=1)
        this = new()
    ...
    function ImagePyramid(im::AbstractArray, t::GaussianPyramid; min_size=15, max_levels=23, filter=[0.0625; 0.25; 0.375; 0.25; 0.0625])
        this = new()
    ...
    function ImagePyramid(im::AbstractArray, t::LaplacianPyramid; min_size=15, max_levels=23, filter=[0.0625; 0.25; 0.375; 0.25; 0.0625])
        this = new()
    ...
    function ImagePyramid(im::AbstractArray, t::SimplePyramid; min_size=15, max_levels=23, filter=[0.0625; 0.25; 0.375; 0.25; 0.0625])
        # im_arr = image_to_array(im)
        im_arr = im
...
function image_to_array(im::Array)
...
function convolve_reflect(im, filter)
  # Zero pad the image in two dimensions, then perform the convolution.
  # Return just the filtered image that sits within the original bounds of the image.
    filter_len = length(filter)
    filter_offset = Int((filter_len-1)/2)

    padded_im = zeros((size(im,1) + filter_len*2), (size(im,2) + filter_len*2))
...
function band_reduce(im; filter=[0.0625; 0.25; 0.375; 0.25; 0.0625])
    new_im_r = convolve_reflect(float64.(red(im)), filter)
    new_im_g = convolve_reflect(float64.(green(im)), filter)
    new_im_b = convolve_reflect(float64.(blue(im)), filter)

    new_im = RGB.(N0f8.(new_im_r), N0f8.(new_im_g), N0f8.(new_im_b))
...
function band_expand(im; filter=[0.0625; 0.25; 0.375; 0.25; 0.0625])
    ...
    new_im_g = convolve_reflect(float64.(green(new_im)), filter)
    new_im_b = convolve_reflect(float64.(blue(new_im)), filter)
    new_im = RGB.(N0f8.(new_im_r), N0f8.(new_im_g), N0f8.(new_im_b))
    return new_im
end
...
function generate_gaussian_pyramid(im; min_size=15, max_levels=23, filter=[0.0625; 0.25; 0.375; 0.25; 0.0625])
    ...
    for i = 1:num_levels
        ...
        im = band_reduce(im, filter=filter)
    end
...
function reconstruct_laplacian_pyramid(pyramid::ImagePyramid)
    ...
    for i = (pyramid.num_levels-1):-1:0
        output_im = band_expand(output_im)
